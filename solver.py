...
class Solver(object):
    ...
    def train_multi(self):
        ...
        x_fixed = x_fixed.to(self.device)
        c_celeba_list = self.create_labels(c_org, self.z_dim, 'CelebA', self.selected_attrs)
        c_rafd_list = self.create_labels(c_org, self.c2_dim, 'RaFD')
        zero_celeba = torch.zeros(x_fixed.size(0), self.z_dim).to(self.device)           # Zero vector for CelebA.
        ...
        for i in range(start_iters, self.num_iters):
            for dataset in ['CelebA', 'RaFD']:
                ...
                if dataset == 'CelebA':
                    ...
                    zero = torch.zeros(x_real.size(0), self.z_dim)
                ...
                # Compute loss with real images.
                out_src, out_cls = self.D(x_real)
                out_cls = out_cls[:, :self.z_dim] if dataset == 'CelebA' else out_cls[:, self.z_dim:]
                d_loss_real = - torch.mean(out_src)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                c_trg_list = self.create_labels(c_org, self.z_dim, self.dataset, self.selected_attrs)
    ...
    def test_multi(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(self.celeba_loader):
                ...
                c_celeba_list = self.create_labels(c_org, self.z_dim, 'CelebA', self.selected_attrs)
                c_rafd_list = self.create_labels(c_org, self.c2_dim, 'RaFD')
                zero_celeba = torch.zeros(x_real.size(0), self.z_dim).to(self.device)            # Zero vector for CelebA.
